...
import { fetchPredictExclude } from '@apis/client/lotto';

type ExcludeItem = { lottoId: number; excludeNumList: number[]; winNums: number[] | null; createdAt: string };
...
const ExcludeTab = () => {
    ...
    const load = async (cursor?: number) => {
        setLoading(true);
        try {
            const res = await fetchPredictExclude({ cursor, limit: 20 });
                ...
                <div key={item.lottoId} css={{ ...panelCss, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    <div css={{ overflowX: 'auto' }}>
                        <div css={{ display: 'flex', gap: 6, minWidth: 'max-content', paddingBottom: 4 }}>
                            {[...item.excludeNumList].sort((a, b) => a - b).map((num) => (
                                <HistoryBall key={num} num={num} isWin={item.winNums?.includes(num) ?? false} />
                            ))}
                        </div>
                    </div>
                </div>
